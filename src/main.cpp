...
std::vector<Pixel_data> create_bitmap_data(const std::vector<int8_t>& field, const std::array<size_t, 2>& size)
{
	...
	for(auto row = size[1]; row > 0; row--){
		for(size_t column = 0; column < size[0]; column++){
			if(field[(row - 1)*size[0] + column] > 0){
				res[(row - 1)*size[0] + column] = white;
			}else{
				res[(row - 1)*size[0] + column] = black;
			}
		}
	}
	return res;
